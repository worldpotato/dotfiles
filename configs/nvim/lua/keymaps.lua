...
-- cmake4vim
map("n", "<leader>xc", "<cmd>:CMake<cr>", options)
map("n", "<leader>xb", "<cmd>:CMakeBuild<cr>", options)
map("n", "<leader>xR", "<cmd>:CMakeReset<cr>", options)
map("n", "<leader>xr", "<cmd>:CMakeRun<cr>", options)
map("n", "<leader>xt", "<cmd>:FZFCMakeSelectTarget<cr>", options)

-- chadtree
map("n", "<leader>v", "<cmd>:CHADopen<cr>", options)

-- terminal
map("t", "<ESC><ESC>", "<C-\\><C-n>", options)
map("n", "<leader>tt", "<cmd>:lua require('FTerm').toggle()<CR>", options)
map("t", "<leader>tt", "<cmd>:lua require('FTerm').toggle()<CR>", options)

-- dap
-- map("n", "<F1>", ":Ultest<CR>", options)
-- map("n", "<F2>", ":UltestNearest<CR>", options)
-- map("n", "<F4>", ":UltestDebugNearest<CR>", options)
map("n", "<F4>", "<cmd>:lua require('dapui').open()<CR>", options)
